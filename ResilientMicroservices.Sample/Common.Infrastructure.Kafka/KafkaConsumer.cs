...
namespace Common.Infrastructure.Kafka
{
    public class KafkaConsumer : HostedService
    {
        ...
        private async Task Consume(Consumer<Ignore, string> consumer, CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    kMessage = ParseAsKMessage(message);
                    ...
                    using (var scope = _serviceProvider.CreateScope())
                    {
                        ...
                        _log.Info($"{nameof(KafkaConsumer)}: received message on topic <{message.Topic}>", kMessage);

                        // 4: The message handler is called to process the actual event
                        await HandleMessage(handler, kMessage, _log, cancellationToken);
        ...
        protected virtual KMessage ParseAsKMessage(Message<Ignore, string> message) =>
            JsonConvert.DeserializeObject<KMessage>(message.Value);
